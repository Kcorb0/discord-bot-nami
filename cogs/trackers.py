...
class CryptoTracker:
    ...
    def c_info(self):
        html_text = requests.get(
            'https://www.coindesk.com/price/{}'.format(self.coin)).text
...
class LolTracker:
    ...
    def lol_rank(self):
        html_text = requests.get(
            'https://lolprofile.net/summoner/euw/{}#update'.format(self.player)).text
        soup = BeautifulSoup(html_text, 'lxml')
        rank = soup.find('div', class_='s-c cf tab1').find('div',
                                                           class_='s-cl').find('span', class_='tier').text
        return str(rank)
    ...
    def lol_games(self):
        html_text = requests.get(
            'https://lolprofile.net/summoner/euw/{}#update'.format(self.player)).text
        soup = BeautifulSoup(html_text, 'lxml')
        wins = (soup.find('div', class_='s-rs-info').find('span',
                                                          class_='win-txt').text).split(" ")[0]
        loss = (soup.find('div', class_='s-rs-info').find('span',
                                                          class_='lose-txt').text).split(" ")[0]
